...
(function($, layer, window, document, undefined){
	function requestData(self,url,param,callback,error){
		$.ajax({
			url:url,
			type:"post",
			data:param,
...
//	增删改查框架
	function CRUD(ele,options){
		...
		this.options.operation = options.operation==null?['a','u','d']:options.operation;
		this.Container = {};
...
//	渲染 执行 方法
	CRUD.prototype.renderUI = function(){
		var container = wrapDiv(this.crud_id,["crud_panel"]);
		...
		var grid = wrapDiv(this.grid_id,["crud_datagrid_panel"]);
		grid.style.width=this.Container.width+"px";
		grid.style.height=this.Container.height/2+"px";
		var form = wrapDiv(this.form_id,["crud_dataform_panel"]);
...
//	渲染数据网格
	CRUD.prototype.renderDataGridUI = function(){
		...
		var toolbar = $.extend(true,[],this.options.datagrid.toolbar);
		...
		if(operation.indexOf("a")!=-1){
			...
			this.options.datagrid.toolbar.push(af);
			this.options.datagrid.toolbar.push('-');
		}
		if(operation.indexOf("u")!=-1){
			var uf = {
				text:"",
				iconCls: 'icon-edit',
				handler: this.toUpdateData
			}
			this.options.datagrid.toolbar.push(uf);
			this.options.datagrid.toolbar.push('-');
		}
		if(operation.indexOf("d")!=-1){
			var df = {
				text:"",
				iconCls: 'icon-remove',
				handler: this.toDeleteData
			}
			this.options.datagrid.toolbar.push(df);
			this.options.datagrid.toolbar.push('-');
		}
		for(var i=0;i<toolbar.length;i++){
			var f = toolbar[i];
			this.options.datagrid.toolbar.push(f);
		}
		if(this.options.datagrid.toolbar.length>2){
			if(this.options.datagrid.toolbar[this.options.datagrid.toolbar.length-1]=="-"){
				this.options.datagrid.toolbar.pop();
			}
		}
...
//	删除数据，接口请求 
	CRUD.prototype.deleteData = function(){
		...
		requestData(self, url,param,callback,error);
	}
